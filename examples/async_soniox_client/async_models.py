...
async def main() -> None:
    ...
    if not api_key:
        raise SystemExit("Please set SONIOX_API_KEY to run the async models example.")

    async with AsyncSonioxClient(api_key=api_key) as client:
        models_response = await client.models.list()
        print(f"(async) Discovered {len(models_response.models)} models:")
        ...
        if first_model.translation_targets:
            print(f"\n(async) Translation targets for {first_model.id}:")
